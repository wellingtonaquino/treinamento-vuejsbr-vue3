<template>
  <div class="flex flex-col px-8 py-6 rounded cursor-pointer bg-brand-gray"
    ...
    <div
      ...
      class="flex mt-4 animate__animated animate__fadeInUp animate__faster"
      v-if="state.isOpen"
    >
      <div class="flex flex-col w-1/2">
        <div class="flex flex-col">
          <span class="font-bold text-gray-400 uppercase select-none">Página</span>
          <span class="font-meduim text-gray-700">{{ feedback.page }}</span>
        </div>
        <div class="flex flex-col mt-4">
          <span class="font-bold text-gray-400 uppercase select-none">Dispositivo</span>
          <span class="font-meduim text-gray-700">{{ feedback.device }}</span>
        </div>
      </div>
      <div class="flex flex-col w-1/2">
        <div class="flex flex-col">
          <span class="font-bold text-gray-400 uppercase select-none">Usuário</span>
          <span class="font-meduim text-gray-700">{{ feedback.fingerprint }}</span>
        </div>
      </div>
    </div>
...
<script>
import Icon from '../Icon/IconConfig'
import Badge from './FeedbackBadge'
